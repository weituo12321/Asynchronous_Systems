-------------------------- MODULE la ------------------------
EXTENDS Naturals, Sequences, cl

CONSTANTS Proc, _ \ll _
...
IN /\ state[p] = "waiting"
   ...
   /\ \A q \in Proc\{p} :
      pReq \prec [TS |-> lastTRcvd[p][q]+1, proc |-> q]
...
Tick(p) ==
/\ \E n \in Nat : /\ n > clock[p]
                  /\ clock' = [clock EXCEPT ![p] = n]
/\ UNCHANGED <<state, msgQ, reqSet, lastTSent, lastTRcvd>>

Next ==
\E p \in Proc : \/ Request(p) \/ Acquire(p) \/ Release(p) \/ \E q \in Proc\{p} : RcvMsg(p, q) \/ Tick(p)

Liveness ==
\A p \in Proc : /\ WF_vars(Acquire(p)) /\ \A q \in Proc\{p} : WF_vars(RcvMsg(p, q))

Spec == Init /\ [][Next]_vars /\ Liveness

DeadlockFree ==
WF_vars(\E p \in Proc : Acquire(p))

Safety ==
\A p \in Proc : \A q \in Proc\{p} : state[p] # "owner" \/ state[q] # "owner"
